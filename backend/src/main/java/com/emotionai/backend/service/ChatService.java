...
public class ChatService {
    ...
    public ChatMessage sendMessage(ChatRequest request) {
        ...
        String activePersona = request.getPersona() != null ? request.getPersona() : "Friend";
        
        Profile profile = profileService.getProfileByUserId(userId);
        QuizResult quiz = quizService.getResultByUserId(userId);
        
        List<ChatMessage> history = chatMessageRepository.findByUserIdAndPersonaOrderByTimestampAsc(userId, activePersona);
        
        String context = buildPromptContext(profile, quiz, history, request.getMessage(), activePersona);
        
        String aiResponse = huggingFaceAiService.generateResponse(context);
        
        ChatMessage message = new ChatMessage();
        message.setUserId(userId);
        message.setPersona(activePersona);
    ...
    public List<ChatMessage> getChatHistory(Long userId, String persona) {
        return chatMessageRepository.findByUserIdAndPersonaOrderByTimestampAsc(userId, persona);
    }

    private String buildPromptContext(Profile profile, QuizResult quiz, List<ChatMessage> history, String currentMessage, String activePersona) {
        ...
        sb.append("You are acting as a caring ").append(activePersona).append(".\n");
        ...
        for (int i = start; i < history.size(); i++) {
            ...
            sb.append(activePersona).append(": ").append(history.get(i).getResponse()).append("\n");
        }
        
        sb.append("User: ").append(currentMessage).append("\n");
        sb.append(activePersona).append(":");
